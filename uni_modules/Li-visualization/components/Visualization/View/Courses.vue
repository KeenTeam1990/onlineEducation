	...
	export default {
		...
		methods: {
			getLatestCourses() {
				...
				uni.vk.callFunction({
					url: 'plugs/client/visualization/pub/getLatestCourses',
					data: params,
					success(data) {
			...
			getRecommendCourses() {
				...
				uni.vk.callFunction({
					url: 'plugs/client/visualization/pub/getRecommendCourses',
					data: params,
					success(data) {
						that.couresList = data.rows
					}
			...
			open(id) {
				uni.vk.navigateTo('/pages/courses/detail?_id=' + id);
			}
		}
	}
</script>
