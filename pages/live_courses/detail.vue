	...
	<view>
		...
		<view class="course-info">
			...
			<view class="sub-info">
				<view>
					<u-icon
						:label="$u.timeFormat(course.start_at, 'yyyy/mm/dd')+' - '+$u.timeFormat(course.end_at, 'yyyy/mm/dd')"
						size="28" label-size='24' name="clock"></u-icon>
				</view>
				<view>
							...
							<view class="video-item" v-for="(item,index) in videos[0]" :key="index"
								...
								<view class="duration">
									...
									<block v-else>
										...
										<block v-else>
											{{item.roome_status?item.roome_status:'未开始'}}
										</block>
									</block>
								</view>
							</view>
	...
	export default {
		...
		methods: {
			...
			confirmShare() {
				...
				uni.setClipboardData({
					data: window.location.host + `/#/pages/live_courses/detail?_id=${this.courseId}&invite=` +
						getApp().globalData.userInfo.my_invite_code,
					success() {
						uni.showToast({
							icon: "none",
							title: "分享链接已复制"
			...
			goVideo(id) {
				uni.vk.navigateTo(`/pages/live_courses/video?course_id=${this.courseId}&video_id=${id}`)
			},
			buy() {
				//先创建订单
				uni.vk.callFunction({
					url: 'client/order/kh/createLiveCourseOrder',
					data: {
						_id: this.courseId
					},
					success(data) {
			...
			getCourse() {
				uni.vk.callFunction({
					url: 'client/live_courses/pub/detail',
					title: "加载中...",
					data: {
						_id: this.courseId,
						need_user_info: true
		...
		onShareTimeline(res) {
			return {
				...
				path: `/pages/live_courses/detail?_id=${this.courseId}&invite=` + getApp().globalData.userInfo
					.my_invite_code,
